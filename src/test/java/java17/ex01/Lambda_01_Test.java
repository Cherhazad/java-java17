...
public class Lambda_01_Test {
    ...
    public void test_filter_by_age() throws Exception {
        ...
        List<Person> result = personList.stream().map(p->p).filter(f -> f.getAge()>=18).toList();
    ...
    public void test_filter_by_firstname() throws Exception {
        ...
        List<Person> result = personList.stream().map(p->p).filter(f -> f.getFirstname().equals("first_10")).toList();
    ...
    public void test_filter_by_password() throws Exception {
        ...
        List<Person> result = personList.stream().map(p->p).filter(f -> f.getAge()>49 && DigestUtils.sha512Hex(f.getPassword()).equals(passwordSha512Hex)).toList();
