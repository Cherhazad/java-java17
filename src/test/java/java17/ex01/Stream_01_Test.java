...
public class Stream_01_Test {
	...
	public void test_stream_filter_adv() throws Exception {
		...
		//List<Order> result = orders.stream().anyMatch(o->"Pépéroni".equals((pizzas.stream().filter(p->p.getName())).toList();
		List<Order> result = orders.stream().filter(order -> order.getPizzas().stream().anyMatch(pizza -> "Pépéroni".equals(pizza.getName()))).toList();
